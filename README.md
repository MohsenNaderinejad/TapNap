...
Approach: Modeling the state as $(u, fuel)$, effectively expanding the graph nodes to $N \times C_{max}$.
...
Approach: Modified Dijkstra's Algorithm where the cost to traverse edge $(u, v)$ is $w_{uv} + \text{wait\_time}(t_{arrival})$.
...
Compute shortest paths from Source ($S$) to all nodes: $d_S[i]$.

Compute shortest paths from all nodes to Destination ($T$) on the transpose graph: $d_T[i]$.

Iterate over the set of Fuel Stations $F$:
...
Description:
A module for real-time analysis of competitor pricing. Supports inserting new price points and querying the rank of a specific price (number of offers $\le P$) in $O(\log N)$ time.
